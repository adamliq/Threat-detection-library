...
data/aria-detections.json.

index.html is the combined library: it embeds both the Splunk SPL (ESXi)
and VMware Aria Operations for Logs catalogues and lets you filter across
both. Run this after editing either data file (adding a new batch, fixing
a field, etc.) to regenerate the static, self-contained index.html that
...
def main():
    ...
    for marker in (MARKER, ARIA_MARKER):
        if marker not in template:
            sys.exit(f"Marker {marker} not found in {TEMPLATE_FILE.name}")

    output = template.replace(MARKER, to_payload(data)).replace(ARIA_MARKER, to_payload(aria_data))
    OUTPUT_FILE.write_text(output, encoding="utf-8")
    print(f"Built {OUTPUT_FILE.relative_to(ROOT)} from {len(data)} Splunk SPL + {len(aria_data)} Aria detection(s).")
